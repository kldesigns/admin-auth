...
.controller('usersCtrl', function($scope, AuthService, API_ENDPOINT, $http, $state, AUTH_EVENTS, $uibModal, notificationsFactory, $log) {
    $scope.$on(AUTH_EVENTS.notAuthenticated, function(event) {
      AuthService.logout();
      $state.go('login');
      var alertPopup = $ionicPopup.alert({
    ...
    $scope.open = function(size) {
      ...
      modalInstance.result.then(function(selectedUser) {
        ...
        if ($scope.user.admin) {
          $scope.user.admin = "yes";
        }
        else {
          $scope.user.admin = "no";
        }
        AuthService.register($scope.user).then(function() {
          notificationsFactory.success("User Created");
          $scope.user = {
            name: '',
            password: '',
            email: '',
          ...
          notificationsFactory.error(errMsg);
        });
      }, function() {
        //they hit cancel so display a cancel message here
        notificationsFactory.info('Cancelled');
        $scope.user = {
          name: '',
          password: '',
          email: '',
    ...
    $scope.passchange = function(size) {

      var modalInstance = $uibModal.open({
        animation: true,
        templateUrl: 'templates/newPasswordModal.html',
        controller: 'NewPasswordModalInstanceCtrl',
        size: size,
        resolve: {
          user: function() {
            return $scope.userpasschange;
          }
        }
      });
      modalInstance.result.then(function(selectedUser) {
        //register user here
        $scope.userpasschange = selectedUser;
        $log.info($scope.userpasschange);
        AuthService.updatepass($scope.userpasschange).then(function(msg) {
          notificationsFactory.success("Password Updated");
          $scope.userpasschange = {
            _id: '',
            name: '',
            password: ''
          };
  ...
  .controller('NewUserModalInstanceCtrl', function($scope, $uibModalInstance, user) {
...
.controller('NewPasswordModalInstanceCtrl', function($scope, $uibModalInstance, user, $http, API_ENDPOINT) {

  $scope.user = user;
  $http.get(API_ENDPOINT.url + '/admin/userlist/').then(function(result) {
...
.controller('clockOnOffCtrl', function($scope, AUTH_EVENTS, AuthService, $state, $ionicPopup, $http, API_ENDPOINT, $log) {
  ...
  $scope.handleUserIDChange = function() {
    var userid = $scope.bundyopts.userid;
    $scope.userselected = 'yes';

    $http.get(API_ENDPOINT.url + '/admin/bundyinfo/' + userid).then(function(result) {
      ...
      if (result.data.bundyState == "off") {
        ...
        $scope.bundyopts.dateon = new Date();
      }
      else {
  ...
  $scope.bundy = function(bundyopts) {
    if (bundyopts.onoff == "on") {
      ...
      $scope.bundyopts.locationon = "admin";

    }
    else {
      $scope.bundyopts.locationoff = "admin";
      //bundyopts.dateoff = new Date();
    }
    $http.post(API_ENDPOINT.url + '/admin/bundy', bundyopts).then(function(result) {
...
.controller('travelLogBookCtrl', function($scope, AUTH_EVENTS, AuthService, $http, $state, $ionicPopup, notificationsFactory, API_ENDPOINT) {
  ...
  $scope.handleUserIDChange = function() {
    self.filter = $scope.users.name;
  };
  $scope.logentry = function(travelopts) {
    $http.post(API_ENDPOINT.url + '/admin/travellog', travelopts).then(function(result) {
      var msg = result.data.msg;
      if (result.data.success) {
...
.controller('userlistingCtrl', function($scope, AUTH_EVENTS, AuthService, $state, $stateParams, $ionicPopup, modalWindowFactory, API_ENDPOINT, $resource) {
  $scope.$on(AUTH_EVENTS.notAuthenticated, function(event) {
    AuthService.logout();
    $state.go('login');
    var alertPopup = $ionicPopup.alert({
  ...
  // API URL  
  self.serverUrl = API_ENDPOINT.url + "/admin/userlisting/";
  //// ---------------- CODE TO RUN -----------
...
.controller('travelreviewCtrl', function($scope, AUTH_EVENTS, AuthService, $state, $stateParams, $ionicPopup, modalWindowFactory, API_ENDPOINT, $resource) {
  ...
  // API URL  
  self.serverUrl = API_ENDPOINT.url + "/admin/travellog/range/" + $scope.searchDate.startDateF + "/" + $scope.searchDate.finishDateF + "/";
  //// ---------------- CODE TO RUN -----------
...
.controller('jobsCtrl', function($scope, AUTH_EVENTS, AuthService, $state, $stateParams, $ionicPopup, modalWindowFactory, API_ENDPOINT, $resource) {
  $scope.$on(AUTH_EVENTS.notAuthenticated, function(event) {
    AuthService.logout();
    $state.go('login');
    var alertPopup = $ionicPopup.alert({
  ...
  // API URL  
  self.serverUrl = API_ENDPOINT.url + "/admin/jobs/";
  //// ---------------- CODE TO RUN -----------
...
.controller('RegisterCtrl', function($scope, $log, AuthService, $ionicPopup, $state) {
  $scope.user = {
    name: '',
    password: '',
    email: '',
...
.controller("itemsController", function($scope, $stateParams, modalWindowFactory, AUTH_EVENTS, AuthService, $ionicPopup, API_ENDPOINT, $resource, $state, $http) {
  $scope.$on(AUTH_EVENTS.notAuthenticated, function(event) {
    AuthService.logout();
    $state.go('login');
    var alertPopup = $ionicPopup.alert({
  ...
  // API URL  
  self.serverUrl = API_ENDPOINT.url + "/admin/timesheet/range/" + $scope.searchDate.startDateF + "/" + $scope.searchDate.finishDateF + "/";
  //// ---------------- CODE TO RUN -----------
